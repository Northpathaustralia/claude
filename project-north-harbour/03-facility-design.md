> **PRIVATE AND CONFIDENTIAL**
> Prepared by James Forycki
> **PROJECT NORTH HARBOUR**
> Not for circulation or distribution without written permission.
>
> *References to Fitness Cartel and other operators are for evaluation as prospective tenants/partners only. Project North Harbour is an independent proposal — not affiliated with, endorsed by, or authorised by Fitness Cartel Australia. No trademark rights are claimed and no operator branding may be used publicly without written agreement.*

# Phases 4–5 — Facility Design Brief & Visualisation Package

**Concept name:** *Project North Harbour — Wellness & Performance Precinct* (operator brand application TBC, subject to executed agreement)
**Positioning:** Australia's benchmark "affordable-luxury" flagship — a health club, recovery house, allied-health hub and social destination on one title, engineered for a catchment that is simultaneously affluent-and-older (Hope Island) and young-and-growing (Oxenford–Coomera corridor).

> **A note on the renders (honesty first):** this package cannot generate photorealistic 3D renders — those are produced by an architectural visualisation studio from the concept design. What this phase delivers is (a) the complete area schedule and design intent an architect needs, and (b) a **shot-by-shot render brief** (§5) ready to hand to a viz studio (typical cost $2.5k–$4k per hero image, ~$35k for the 12-image pack). Schematic site/floor plans are included in the investor deck (`deck/index.html`).

---

## 1. Development summary

| Parameter | Value |
|---|---|
| Site | 10,000m² benchmark parcel, Hope Island Rd gateway |
| Built GFA | **6,200m²** over 2 levels + mezzanine (within the 4,500–7,000m² brief) |
| Club footprint | 4,100m² ground + 2,100m² level 1 |
| Allied-health pads (leased) | 600m² in-building + provision for 2 future pad sites |
| Parking | ~290 bays (incl. 20 EV, 8 PWD) + 60 bicycle + coach bay |
| Outdoor training | 450m² turf/rig zone + 25m outdoor lap-pool option (Stage 2) |
| Members (design capacity) | 5,500 with peak-hour comfort at 480 concurrent |

## 2. Area schedule (6,200m² GFA)

### Ground floor — 4,100m²
| Zone | m² | Design intent |
|---|---|---|
| Arrival, luxury reception & lounge | 260 | Double-height, hotel-lobby language: stone, timber battens, brass signage wall (operator brand TBC); concierge not counter |
| Café & juice bar (licensed, street-trading) | 180 | Trades to street AND club; 60 seats + terrace; own ABN/lease possible |
| Retail — supplements, apparel, recovery tech | 90 | Adjacent reception, impulse capture |
| Strength zone (free weights) | 700 | 40+ benches/racks zone, 6 deadlift platforms, competition kit |
| Powerlifting / performance annex | 180 | 4 combo racks, calibrated plates, chalk-permitted, acoustic slab isolation |
| Bodybuilding pin/plate-loaded | 450 | Full dual line-up of premium plate-loaded (Panatta/Prime/Watson tier) |
| Functional / HYROX / cross-training hall | 520 | 30m sled track, rig, comp-standard erg fleet; hosts HYROX-affiliate events |
| Group fitness studio 1 (60 pax) | 180 | Les-Mills-scale, sprung floor |
| Cycle studio (40 bikes) | 120 | Immersive AV, theatre rake |
| **Recovery centre** | **520** | Cold plunges (2× communal, 2× private), magnesium mineral pools, 2 traditional saunas + 1 infrared suite, steam room, contrast deck, compression-boot lounge (12 stations), quiet room — *club-integrated version of the Merse playbook* |
| Wet change / luxury bathrooms | 320 | Hotel-grade: stone vanities, private cubicles, day-lockers, towel service |
| Kids club | 160 | Licensed capacity ~40, own amenities, secure drop-off from carpark |
| Physio / sports-massage / allied suites (leased) | 300 | 8 consult rooms + rehab gym window onto main floor |
| BOH, plant, loading, circulation | 120 | + roof plant deck |

### Level 1 — 2,100m²
| Zone | m² |
|---|---|
| Reformer Pilates studio A (28 beds) + studio B (16 beds) | 340 |
| Yoga / mind-body studio (hot-capable) | 160 |
| Cardio mezzanine overlooking gym floor | 450 |
| Stretch & mobility deck | 150 |
| Co-working lounge + corporate meeting rooms (2) | 220 |
| Podcast studio + 2 content-creation rooms | 90 |
| Private PT studios (4) | 160 |
| Admin, staff, training academy | 180 |
| Members' outdoor terrace (covered) | 200 |
| Circulation/amenities | 150 |

**Deliberate exclusion:** a full basketball court was tested and **rejected** — 700m²+ of low-yield, high-volume space serving the weakest segment of this catchment. The functional hall accepts a single adjustable hoop; the land bank supports a future court pavilion if member data demands it. (This is the kind of trade-off that makes the pro-forma credible.)

## 3. Architecture & experience standards

- **Façade:** 9m glass "engine room" window to Hope Island Rd showing the strength floor in motion after dark — the building is the billboard. Charred-timber-look cladding, off-form concrete blades, landscaped berm; **12m illuminated pylon (operator brand TBC)** at the arterial corner (sign code permitting).
- **Entrance:** porte-cochère arrival, 6m brass-lettered portal, water-feature + native landscape (Gold Coast City landscape code species).
- **Interior:** black-steel + walnut + travertine palette; 4.5–7m ceilings on gym floor; circadian-tuned lighting zones (bright performance / warm recovery); club-wide scent and sound design; premium acoustic treatment (NRC ≥ 0.8 over strength zones).
- **Technology:** app-gated 24/7 access, per-zone occupancy heat-mapping shown in-app, Technogym-ecosystem cardio, body-scan suite, EV chargers, 300–400kW rooftop solar + battery, heat-pump pool heating targeting 5-star NABERS-equivalent running costs.

## 4. Staging

- **Stage 1 (open day one):** everything in §2.
- **Stage 2 (year 2–4, land-banked):** 2 × 300m² allied-health/medical pad buildings; optional 25m outdoor magnesium lap pool + swim school (adds ~$2.8m, unlocks learn-to-swim revenue); optional court pavilion.

## 5. Render brief — 12 hero images (for viz studio / DA package)

| # | Shot | Brief |
|---|---|---|
| 1 | Street hero, dusk | 3-quarter from Hope Island Rd median; glowing strength-floor window, pylon lit, traffic light-trails |
| 2 | Aerial drone | Full precinct incl. parking, outdoor zone, station in background, canal estates beyond |
| 3 | Porte-cochère entry | Evening arrival, landscape lighting, brass portal |
| 4 | Reception/lounge | Double-height, café beyond, morning light |
| 5 | Strength floor | Wide from mezzanine, full rack lines, engine-room drama |
| 6 | Recovery centre | Magnesium pool + plunge court, steam glass, spa-grade mood |
| 7 | Reformer Pilates A | 28 beds, level-1 glazing to terrace |
| 8 | Café terrace | Licensed terrace trading to street, families + lycra |
| 9 | HYROX hall | Sled track in use, event-day dressing |
| 10 | Night exterior | Long exposure across carpark, solar canopy visible |
| 11 | Kids club | Secure entry, parents' sightline |
| 12 | Co-working lounge | Laptops + smoothies, corporate-membership sell |

Renders use a neutral "North Harbour" placeholder identity until an operator heads-of-agreement and brand licence are executed — only then is the confirmed operator's brand kit applied (under NDA and their brand guidelines). No operator trademark appears in any public-facing material.

## 6. Why this design wins this catchment

1. **~35% of GFA is recovery/Pilates/allied/social** — the highest-margin, least-Instagram-perishable revenue lines, matched to a median-age-50, $100k-household catchment.
2. **Serious iron credibility is retained** (700m² free weights + powerlifting annex) so the brand keeps its base and the 25–45 corridor members driving in from Oxenford/Coomera.
3. **Daytime yield:** retirees + shift-flexible members flatten the 6pm peak that kills big-box economics; co-working and café monetise the 10am–3pm trough.
4. **The building is defensible:** a competitor can copy a weights floor in 6 months; they cannot copy a piled, tanked, pooled recovery centre and licensed café precinct without $30m+ and 2 years.
